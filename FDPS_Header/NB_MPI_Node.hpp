#ifndef NBMPINODE_HPP
#define NBMPINODE_HPP
...
template <class ObjT1, class ObjT2, class functor11(), class functor12(), class functor22()>
class NB_MPI_Node : MPI_Node<double> {
    // the node type for neighbor operations.
    // inherit from MPI_Node<double> for mpi NB operations
    // double precision only

  public:
    using Node_t = MPI_Node<double>; // Type of base node class (MPI_Node / Cheb_Node)
    ...
    class NodeData : public Node_t::NodeData {
      public:
        std::vector<ObjT1> *objT1Ptr;
        std::vector<ObjT2> *objT2Ptr;
    };
     ...
     */
    NB_MPI_Node() {
        Node_t(); // the base class constructor
        fmm_data = nullptr;
    }
     ...
     * saved.
     */
    virtual void NodeDataVec(std::vector<Vector<Real_t> *> &coord, std::vector<Vector<Real_t> *> &value,
