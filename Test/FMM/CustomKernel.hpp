...
inline const Kernel<T> &StokesCustomKernel<T>::DoublePressure() {
    static Kernel<T> potn_ker =
        BuildKernel<T, stokes_double_pressure<T, 1>>("stokes_double_pressure", 3, std::pair<int, int>(9, 1));
    return potn_ker;
}
...
inline const Kernel<double> &StokesCustomKernel<double>::DoublePressure() {
    typedef double T;
    static Kernel<T> potn_ker =
        BuildKernel<T, stokes_double_pressure<T, 2>>("stokes_double_pressure", 3, std::pair<int, int>(9, 1));
    return potn_ker;
}
