...
// requires c++11 variadic template
template <typename... ObjTypes> // e.g. MyClass<T1, T2>
class NBNode {
  ...
  protected:
    ...
    std::tuple<std::vector<ObjTypes> *...> objContainerPtrs; // pointers to container of objects
    std::array<std::vector<size_t>, sizeof...(ObjTypes)>
        objIndexArray;              // indices of objs owned by this node (including its child nodes)
    std::vector<size_t> pointIndex; // the index of objs in the fist container, used for tree subdivision
    std::vector<double> pointCoord; // the coordinate of objs in the first container, used for tree subdivision

    Buffer packedData; // packed data for transfer over MPI

    // constructor
    NBNode(int maxDepth_, std::tuple<std::vector<ObjTypes> *...> & objContainerPtrs_) : maxDepth(maxDepth_<MAXTREEDEPTH?maxDepth_,MAXTREEDEPTH), 
    parent(nullptr), child(nullptr),objContainerPtrs(objContainerPtrs), status(1) {
     ...
     * this->getParent()->getChild(this->Path2Node())==this
     */
    int Path2Node();
     ...
     * saved.
     */
    void NodeDataVec(std::vector<Vector<Real_t> *> &coord, std::vector<Vector<Real_t> *> &value,
